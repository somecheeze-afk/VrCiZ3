// Quiz app.js (v11)
// - manifestUrl を window.QUIZ_CONFIG.manifestUrl で指定可能（例: "./quizzes/manifest.json"）
// - 404 等で読めない場合は "./manifest.json" をフォールバック
// - 選択肢シャッフル（answer追従）対応
// - 一覧に「開始」「PDF」ボタン表示
...
(() => {
  ...
  const CONFIG = Object.assign({
    shuffleChoices: true,
    manifestUrl: "./quizzes/manifest.json"
  }, (window.QUIZ_CONFIG || {}));
  ...
  async function fetchJson(url){
    ...
    if(!r.ok) {
      const err = new Error(`${url} の読み込みに失敗しました (${r.status})`);
      err.status = r.status;
      throw err;
    }
    return r.json();
  ...
  function resolveQuizJsonUrl(meta){
    // manifest 側のキー差異を吸収
    return meta.file || meta.json || meta.path || meta.url || (meta.id ? `./quizzes/${meta.id}.json` : null);
  }
  ...
  async function openQuiz(meta){
    ...
    const jsonUrl = resolveQuizJsonUrl(meta);
    if(!jsonUrl) throw new Error("クイズJSONの場所が manifest にありません（file/json/path/url/id）。");
    ...
    const header = el("div", { class:"header-row", style:"align-items:center" },
      el("div", {},
        ...
        el("div", { style:"opacity:.75; margin-top:6px" }, `Score: `, el("span", { id:"score", style:"font-weight:800" }, "0"), ` / ${total}`)
      ),
